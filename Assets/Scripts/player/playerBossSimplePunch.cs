...
public class playerBossSimplePunch : MonoBehaviour
{
    public Animator animator;

    public Transform punchPoint; // the point of attack
    public float punchRange = 0.08f; // the range of punch attack
    public int punchDamage = 20; // the damage value of punch attack
    ...
    foreach(Collider2D enemy in hitEnemies)
        {
            enemy.GetComponent<bossHP>().takeDamage(punchDamage);
        }
    }
}
